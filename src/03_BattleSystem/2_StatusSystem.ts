...
import data_Debufs from '../05_Data/bufsNdebufs.json' with { type: 'json' };

// 상태이상 부여 함수 (Manager)
export function TryApplyStatus(target: Pokemon, statusTag: string): boolean {
    
    // 1. 이미 상태이상이 있으면 실패
    if (target.BattleState.Get() !== "Normal") {
...
export function ResolveStatusEffects(pokemon: Pokemon): void {
    ...
    if (statusData.dmg) {
        const damage = Math.floor(pokemon.Stats.maxHp * statusData.dmg);
        ...
        pokemon.Stats.takeDamage(finalDamage);
    }
}
