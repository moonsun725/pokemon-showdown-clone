...
export function calculateDamage(attacker: Pokemon, defender: Pokemon, move: Move): DamageResult {
    ...
    // 데미지 계산
    const abilityMod = attacker.ability.GetDamageMod(move, basePower);
    // 예: 생명의구슬(Life Orb) -> 1.3배
    const itemMod = attacker.item.GetDamageMod(move, basePower); // ItemManager도 비슷하게 구현 필요
